...
public class Main {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for(int i=0;i<N;i++) {
			for(int j=0;j<N;j++) {
				if(visited[i][j]==false) {
					answer1++;
					bfs(1, arr, visited, new int[] {i,j});	//비 적록색약
				}
			}
		}
		...
		for(int i=0;i<N;i++) {
			for(int j=0;j<N;j++) {
				if(visited[i][j]==false) {
					answer2++;
					bfs(2,arr, visited, new int[] {i,j});	//적록색약
				}
			}
		}
	...
	public static void bfs(int type, char [][]arr, boolean [][]visited, int[] destination) {	//비 적록색약
		int []dx = {-1,1,0,0};
		int []dy = {0,0,-1,1};
		...
		while(!queue.isEmpty()) {
			...
			for(int i=0;i<4;i++) {
				...
				if(type==1 && visited[nx][ny]==false && arr[nx][ny]==arr[destination[0]][destination[1]]) {	//비 적록색약
					queue.add(new int[] {nx, ny});
					visited[nx][ny]=true;
				}
				else if(type==2 && visited[nx][ny]==false) {	//적록색약
					if((arr[destination[0]][destination[1]] == 'B' && arr[nx][ny]=='B') || (arr[nx][ny]!='B' && arr[destination[0]][destination[1]]!='B')) { //B일 경우, B가 아닐 경우로 나눔
						queue.add(new int[] {nx, ny});
						visited[nx][ny]=true;
					}
				}
